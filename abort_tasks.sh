# abort_tasks.sh [budget]

if [ $# -lt 1 ]
then
	echo "run_test.sh [budget]"
	exit
fi
...
do
	...
	ssh "root@"$remoteVM "screen -X quit"
	#ssh "root@"$remoteVM "rm -rf testSuites"$i"_"$budget "results"$i"_"$budget
	#ssh "root@"$remoteVM "rm -rf testSuites* results*"
	#ssh "root@"$remoteVM hostname
done

echo "* abort_tasks Complete!"
