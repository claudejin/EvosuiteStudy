...
do
	...
	scp gen_test.sh "root@"$remoteVM":/root/"
	#scp "evosuite-master-1.0.4-w1.jar" "root@"$remoteVM":/root/defects4j/framework/lib/test_generation/generation/evosuite-d-1.0.4.jar"
	#ssh "root@"$remoteVM "./setting.sh"
	#if [ "$i" -lt 25 ]
	#then
	#	ssh "root@"$remoteVM "./setting.sh k"
	#else
	#	ssh "root@"$remoteVM "./setting.sh d"
	#fi
done
